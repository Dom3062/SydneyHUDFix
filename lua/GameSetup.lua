if GameSetup then
    ...
    CustomWaypoint.OFFSCREEN_TYPE = table[math.clamp(SydneyHUD:GetOption("offscreen_type"), 1, 3)]	--circle, ellipse, border
    CustomWaypoint.OFFSCREEN_RADIUS_SCALE = SydneyHUD:GetOption("offscreen_radius_scale")	--Radius multiplier for circle/ellipse offscreen size
    CustomWaypoint.TRANSIT_SPEED = SydneyHUD:GetOption("transit_speed")			--Transition time from offscreen position to onscreen position when not using border offscreen mode
