...
function HUDTeammate:_init_stamina_meter()
    ...
    if managers.player:upgrade_value("player", "armor_max_health_store_multiplier", 0) > 0 or managers.player:upgrade_value("player", "armor_health_store_amount", 0) > 0 then
        stamina_size = 0.3
    end
...
function HUDTeammate:update_hps_meter(current_hps, total_hps)
    if self._hps_meter then
        if SydneyHUD:GetOption("enable_hps_meter") and ((SydneyHUD:GetOption("show_hps_current") and current_hps and current_hps > 0) or (SydneyHUD:GetOption("show_hps_total") and total_hps and total_hps > 0)) then
